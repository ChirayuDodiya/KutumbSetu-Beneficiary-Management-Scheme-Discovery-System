...
import { Upload, PlusCircle, CheckCircle, FileText, Trash2, LayoutList } from 'lucide-react';
...
const AdminDashboard = () => {
  ...
  const handleDelete = async (id) => {
    if (!window.confirm('Are you sure you want to delete this scheme and its AI memory?')) return;
    try {
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="flex justify-end pt-4">
              <button type="submit" disabled={loading} className="bg-indigo-600 hover:bg-indigo-700 text-white px-6 py-2 rounded font-medium disabled:opacity-50">
                {loading ? 'Uploading...' : 'Create Scheme'}
              </button>
            </div>
          </form>
        </div>
